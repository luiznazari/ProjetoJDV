...
import br.edu.unoesc.exception.JdvException;
...
import br.edu.unoesc.rede.modelo.JdvRede;
import br.edu.unoesc.rede.modelo.JdvRedeAbstrata;
import br.edu.unoesc.rede.modelo.MapaSaida;
...
public class JdvUtils {
	...
	public static class Tabuleiro {
		...
		public static void comparaJogadores(JogadorAutomato um, JogadorAutomato dois,
			...
			System.out.println("Simulação finalizada. Tempo total: " + Duration.between(inicio, LocalTime.now()));
		}
	...
	public static class RNA {
		...
		public static void converteArquivosDeDadosEntreRedes(JdvRedeAbstrata redeBase,
			...
			dados.getConjuntos().stream().forEach(par -> {
				double[] entradasBase = redeBase.converteEntradaEmTabuleiroNormalizado(par.getPar().getInput());
				MLData entradasObjetivo = redeObjetivo.traduzirEntrada(entradasBase);

				int saidaBase = redeBase.traduzirSaida(par.getPar().getIdeal());
				MLData saidaObjetivo = redeObjetivo.convertePosicaoTabuleiroEmSaida(saidaBase);

				JdvMLDataPair novoPar = new JdvMLDataPair(entradasObjetivo, saidaObjetivo,
						par.getFrequencia(), par.getPontos());
				dadosRedeObjetivo.add(novoPar.getParaSalvar());
			});
